...
# def _get_edges(cell) -> List:
#     cell_as_graph = to_graph(cell)
#     return list(cell_as_graph.edges)

# def _compute_g_ij(i, j, lengths_CM, radii_CM, R_L) -> float:
#     """
#     Compute axial coupling conductance density g_{ij} in S/cm^2.

#     This is the coefficient multiplying (V_j - V_i) in compartment i's membrane
#     equation when using per-area (density) units, assuming cylindrical compartments.

#     Formula:
#         g_{ij} = (a_i * a_j^2) / (R_L * L_i * (L_i * a_j^2 + L_j * a_i^2))

#     where:
#         L_k = lengths[k]  (cm)
#         a_k = radii[k]    (cm)
#         R_L = axial resistivity R_a (Ohm·cm)

#     Returns:
#         float: g_{ij} in S/cm^2.

#     Raises:
#         ValueError: if indices are equal or if any geometric/physical parameter is nonpositive.
#         IndexError: if i or j are out of bounds.
#     """
#     if i == j:
#         raise ValueError("i and j must be different compartments.")

#     # Extract and validate
#     L_i = float(lengths_CM[i]); L_j = float(lengths_CM[j])
#     a_i = float(radii_CM[i]);   a_j = float(radii_CM[j])
#     R_a = float(R_L)
...
#     if R_a <= 0:
#         raise ValueError("Axial resistivity R_L must be positive.")

#     denom = R_a * L_i * (L_i * a_j * a_j + L_j * a_i * a_i)
#     return (a_i * a_j * a_j) / denom

# def _build_axial_matrix(edges, lengths_CM, radii_CM, axial_resistivities):
#     """
#     M[i,i] = -sum_{j connected to i} g_{i,j}
#     M[i,j] = g_{i,j}  if (i,j) is an edge (each undirected pair appears once)
#     Finally, scale each row i by surface_areas[i].
#     """
#     ncomps = len(lengths_CM)
#     M = jnp.zeros((ncomps, ncomps), dtype=lengths_CM.dtype)

#     for i, j in edges:
#         if i == j:
#             continue
#         R_a = axial_resistivities[i]
#         # g_{i,j} density applied to i, and g_{j,i} density applied to j
#         g_ij = _compute_g_ij(i, j, lengths_CM, radii_CM, R_a)
#         g_ji = _compute_g_ij(j, i, lengths_CM, radii_CM, R_a)

#         # Accumulate diagonals
#         M = M.at[i, i].add(-g_ij)
#         M = M.at[j, j].add(-g_ji)

#         # Assign off-diagonals (each pair appears once)
#         M = M.at[i, j].set(g_ij)
#         M = M.at[j, i].set(g_ji)

#     # Compute surface areas of all the compartments
#     surface_areas_CM2 = get_surface_areas(lengths_CM, radii_CM)
#     # Row-scale by surface areas: row i *= surface_areas[i]
#     M = M * surface_areas_CM2[:, None]
#     return M

# def build_axial_matrix(cell):
#     # get edges
#     edges = _get_edges(cell)
#     # get lengths and radii of all the compartments
#     radii_CM = cell.nodes["radius"].to_numpy()*1e-4
#     lengths_CM = cell.nodes["length"].to_numpy()*1e-4
#     # build axial matrix
#     axial_resistities = cell.nodes["axial_resistivity"].to_numpy()
#     axial_matrix = _build_axial_matrix(edges, lengths_CM, radii_CM, axial_resistities)
#     return axial_matrix

# def compute_eap_t(voltages_t, axial_matrix, current_t, idx_stimulus, distances_CM, extracellular_resistivity):
#     """
#     voltages_t: (n_comp,) [mV]
#     axial_matrix: (n_comp, n_comp); row-area–scaled so (axial_matrix @ voltages_t) -> currents [mA]
#     distances_CM: (n_elec, n_comp) [cm], must be > 0
#     extracellular_resistivity: ρ [Ω·cm]
#     returns: (n_elec,) EAP [mV]
#     """
#     current_mA = current_t * 1e-6
#     trans_current = axial_matrix @ voltages_t           # (n_comp,), [mA]
#     trans_current = trans_current.at[idx_stimulus].add(current_mA)
#     scaling = extracellular_resistivity / (4.0 * jnp.pi)
#     eap = scaling * ((1.0 / distances_CM) @ trans_current)  # (n_elec,)
#     return eap

# def compute_eap(voltages, axial_matrix, current, idx_stimulus, distances_CM, extracellular_resistivity):
#     """
#     voltages: (n_comp, T) [mV]
#     axial_matrix: (n_comp, n_comp) -> (A @ voltages) gives currents [mA]
#     distances_CM: (n_elec, n_comp) [cm], must be > 0
#     extracellular_resistivity: ρ [Ω·cm]
#     returns: (n_elec, T) [mV]
#     """
#     current_mA = jnp.concatenate((jnp.array([0.]),current * 1e-6))
#     trans_current = axial_matrix @ voltages                            # (n_comp, T) [mA]
#     trans_current = trans_current.at[idx_stimulus, :].add(current_mA)  # add injection at that compartment
#     scaling = extracellular_resistivity / (4.0 * jnp.pi)
#     inv_r = 1.0 / distances_CM                           # (n_elec, n_comp)
#     return scaling * (inv_r @ trans_current)  # (n_elec, T)
...
# def compute_eap(voltages, A_mat, current, idx_stimulus, distances_CM, extracellular_resistivity):
#     """
#     Compute extracellular potentials from compartment voltages using a prebuilt
#     linear map A such that I_mem [mA] = A @ V [mV].

#     Args
#     ----
#     voltages : (n_comp, T) array, mV
#         Compartment voltages over time.
#     A_mat : (n_comp, n_comp) array, mA/mV
#         Matrix from build_A_from_edge_coeffs(...). Maps voltages (mV) to
#         outward transmembrane currents (mA).
#     current : (T,) array-like or None, nA
#         Injected intracellular current time-series applied at idx_stimulus.
#         Use None if no injection term is needed.
#     idx_stimulus : int or None
#         Compartment index where `current` is applied. Ignored if `current` is None.
#     distances_CM : (n_elec, n_comp) array, cm
#         Electrode–compartment distances; strictly positive.
#     extracellular_resistivity : float, Ω·cm
#         Medium resistivity ρ.

#     Returns
#     -------
#     eap : (n_elec, T) array, mV
#         Extracellular potentials at each electrode over time.
#     """
#     # 1) transmembrane currents via the linear map (mA)
#     #current = jnp.concatenate((jnp.array([0.]),current))
#     I_mem_mA = transmembrane_current(
#         A_mat,
#         voltages,
#         Iinj_nA = current if current is not None else None,
#         inj_index=idx_stimulus,
#     )  # shape: (n_comp, T)

#     # 2) Free-space superposition: φ_e = (ρ/4π) * Σ_i I_i / r_ei  → mV
#     scaling = extracellular_resistivity / (4.0 * jnp.pi)           # Ω·cm / (–) 
#     inv_r   = 1.0 / jnp.asarray(distances_CM)                      # (n_elec, n_comp) 1/cm
#     eap_mV  = scaling * (inv_r @ I_mem_mA)                         # (n_elec, T) mV
